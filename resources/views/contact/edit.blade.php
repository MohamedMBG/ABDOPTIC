        ...
        <div class="row">
            <div class="col-md-4">
                @if(!empty($contact->image))
                    <div class="thumbnail">
                        <img src="{{ asset('storage/' . $contact->image) }}" alt="Contact Image" class="img-responsive" style="max-height: 150px;">
                        <div class="caption">
                            <p class="text-center">
                                <a href="#" class="btn btn-danger btn-xs delete-image" data-contact-id="{{ $contact->id }}">
            ...
            <div class="col-md-8">
                {!! Form::label('image', __('lang_v1.contact_image') . ':') !!}
                <div class="input-group">
                    <span class="input-group-addon">
                ...
                <p class="help-block">
                    @lang('lang_v1.image_filetypes')<br>
                    @lang('lang_v1.max_file_size', ['size' => '2MB'])
                </p>
            </div>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    $('.delete-image').click(function(e) {
        ...
        if (confirm('{{ __("messages.confirm_delete_image") }}')) {
            $.ajax({
                url: '/contacts/' + contactId + '/delete-image',
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
